...
export const processComparison = async (req, res) => {
    try {
        ...
        const isFresh = cachedProduct && (Date.now() - new Date(cachedProduct.lastScraped).getTime() < ONE_DAY);
        ...
        if (originalData && originalData.success && !comparisonData) {
            console.log("No competitor found in cache. Starting cross-platform search.");
